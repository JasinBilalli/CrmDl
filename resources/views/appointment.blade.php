...
<div class="col-lg-3 col-12 box follow-scroll">
    ...
    @if($appointment["rejected"] == 0)
      <div class='fc-event' style="margin: 10px 0; cursor: pointer; border: 1px solid #ccc;   font-size: 12px;  background: #f8deca;">{!! $appointment["id"] !!}-{!! $appointment['first_name'] !!} {!! $appointment['last_name'] !!}<br><B style="color: #5e0b0b">{!! date("d/M/Y H:i", strtotime($appointment["appointment_date"]." ".$appointment["time"])) !!}({!! $appointment["number_of_persons"] !!} persons)</B><br>@if($appointment["assigned"] == '0')<a href='acceptappointment/{!! $appointment["id"] !!}' style="color:red">Not assigned </a>@endif</div>
      @else
      <div class='fc-event' style="margin: 10px 0; cursor: pointer; border: 1px solid #ccc;   font-size: 12px;  background:rgb(255, 0, 0,0.55);">{!! $appointment["id"] !!}-{!! $appointment['first_name'] !!} {!! $appointment['last_name'] !!}<br><B style="color: #5e0b0b">{!! date("d/M/Y H:i", strtotime($appointment["appointment_date"]." ".$appointment["time"])) !!}({!! $appointment["number_of_persons"] !!} persons)</B><br>@if($appointment["assigned"] == '0')<a href='acceptappointment/{!! $appointment["id"] !!}' style="color:red">Rejected </a>@endif</div>
@endif
	  @endforeach
